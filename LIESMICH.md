...
Diese Regeln stehen **fertig mit deiner Admin-Mail** in der Datei `firestore.rules` –
einfach von dort kopieren. Inhalt:
...
**6. Als Admin anmelden** – am einfachsten mit **„Mit Google anmelden"** und deiner
Admin-Gmail (gilt sofort als bestätigt). Alternativ einen Account per E-Mail/Passwort
anlegen (**Authentication → Users → Add user**) und die zugeschickte **Bestätigungs-Mail**
öffnen. Danach erscheint oben der Button **Admin**.
...
- Beide öffnen das E-Mail-Programm mit Empfänger = erste Adresse aus `ADMIN_EMAILS`. Bewusst
  **kein** Server dazwischen (keine Angriffsfläche). Soll es vollautomatisch ohne
  Mail-Programm senden, lässt sich das mit einem Dienst wie EmailJS/Web3Forms nachrüsten.
...
- **Keine Rechte-Eskalation:** Admin-Status hängt nur an `ADMIN_EMAILS` + Regeln, nicht an
  einem Feld in der Datenbank.
