...
function save(){
  ...
  }catch(e){ /* Speichern nicht möglich (z. B. localStorage deaktiviert) – App läuft trotzdem */ }
}
function load(){
  try{
...
function renderAll(){ renderStats(); renderStrategy(); renderMoney(); }
...
function resetAll(){
  if(!confirm("Wirklich ALLES zurücksetzen (Zählung, Session, Einstellungen)?")) return;
...
function init(){
  ...
  $("currency").addEventListener("input", e=>{ state.currency=e.target.value||"CHF"; save(); renderAll(); });
