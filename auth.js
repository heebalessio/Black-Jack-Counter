...
/* ---------- Kontakt/Anfrage per E-Mail (mailto – kein Server, keine Angriffsfläche) ---------- */
...
function sendContact(){
  const to = (ADMIN_EMAILS && ADMIN_EMAILS[0]) ? ADMIN_EMAILS[0] : "";
  ...
  closeContact();
  window.location.href = url;   // öffnet das Mail-Programm (alles URL-kodiert)
}
...
async function flushSave(){
  ...
  try{
    await setDoc(doc(db, "users", u.uid),
      { state: data, email: u.email, lastWriter: CLIENT_ID, updatedAt: serverTimestamp() }, { merge: true });
  }catch(e){ console.warn("Speichern in der Cloud fehlgeschlagen:", e); }
}

/* ---- Live-Synchronisation: Änderungen von ANDEREN Geräten sofort übernehmen ---- */
let liveUnsub = null;
...
onAuthStateChanged(auth, async (user) => {
  ...
  if(!snap.exists()){
    // Kein vom Admin angelegtes Profil → KEIN Zugang. Nur Admins (per ADMIN_EMAILS)
    // legen sich beim ersten Login selbst an. So kann sich NIEMAND selbst registrieren.
    if(isAdminEmail(user.email)){
      await setDoc(ref, { email:user.email, role:"admin", state:{}, createdAt: serverTimestamp() });
  ...
  // Admin-Status NUR über ADMIN_EMAILS (deckt sich mit den Firestore-Regeln) –
  // bewusst NICHT dem role-Feld im Dokument vertrauen (sonst Selbst-Hochstufung möglich).
  currentRole = isAdminEmail(user.email) ? "admin" : "user";
